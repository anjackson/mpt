...
This will build an executable in the `dist` folder. But note that this has only been tested under Gooey 1.0.4 on Windows 10. Currently, Gooey is not compatible with macOS Catalina (because only newer versions of WXPython are supported there) and so that won't work until `1.0.5 is released <https://github.com/chriskiehl/Gooey/issues/586>`_.
